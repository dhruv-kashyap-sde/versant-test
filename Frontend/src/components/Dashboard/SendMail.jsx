...
const SendMail = ({ isAdmin }) => {
    ...
    if (isAdmin) {
      ...
      try {
        ...
        const response = await axios.post(
          `${import.meta.env.VITE_API}/trainer/students`,
          { token }
        );
  ...
  const handleChangePage = (event, newPage) => {
    ...
    const allNewPageStudentsSelected = newPageStudents.every((student) =>
  ...
  const sendInvitationEmails = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success(
          `Invitations sent to ${selectedStudents.length} students`
        );
        setSelectedStudents([]);
        setSelectAll(false);
      }
