...
export const F = () => {
  ...
  const fetchQuestions = async () => {
    setIsLoading(true);
    try {
      let response = await axios.get(`${import.meta.env.VITE_API}/questions/part?part=F`);
  ...
  const handleDeleteQuestion = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API}/questions/${id}?part=F`);
  ...
  return (
    <Box>
      <Typography variant="h6" component="h3" gutterBottom color="primary" fontWeight="medium">
        Part F: Candidates reconstruct the content of a short passage presented for 30 seconds
      </Typography>
      ...
      <ExampleBox elevation={2}>
        ...
        <Box sx={{ mb: 2 }}>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
            <EyeIcon color="primary" sx={{ mt: 0.5 }} />
        ...
        <Box>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
            <PencilIcon color="primary" sx={{ mt: 0.5 }} />
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell width="75%">Passage</StyledTableCell>
